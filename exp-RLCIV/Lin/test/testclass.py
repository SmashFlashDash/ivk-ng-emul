...
from test.test_classes import Input, Device_arguments, UV_arguments, QurDB, Funcs, WrapEx, bcolors
##############################################################################################
...
class Test():
    ...
    def di_lst_custom(self, *args):
        ...
        for idx, cypher in enumerate(di_list): 
            ...
            if newK is None:
                ...
                cypher = cypher[:-2]
            di_dict[cypher+parsK] = [newK] + [None, None, None] + [timequr] + [None, None, None]
            list_qur.append(cypher+parsK)
        # Запрос к БД
        ret_di = QurDB.qur_tmi(di_dict, ret_di, list_qur)
        info_di = []
    ...
    def _quest_uv_di(self):
        if self.UV.uv_equation is None:
            ...
            self.UV.res, self.UV.info_tmi, ret_di = WrapEx.ex_wait_v2(self.UV)
