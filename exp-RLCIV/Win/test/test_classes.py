...
class UV_arguments():
    ...
    def set_uv_arguments(self, UV, device, t_TMI, t_uv, di_warnlist, tmi_ExCH, hex_data, subadress, msgkey):
        ...
        if args_uv[uv_fields[1]] is not None and subadress == False:
            self.uv = args_uv[uv_fields[1]]
            self.uv_bin = self.uv
            self.uv =  int(bin(self.uv)[7:], 2)
        else:
            if subadress == False:
        ...
        # self.type = args_uv[uv_fields[2]]

        self.rv_uvs = args_uv[uv_fields[5]]
        if isinstance(self.rv_uvs, str): self.rv_uvs = [self.rv_uvs,]
        ...
        if isinstance(self.msg_keys, str): self.msg_keys = [self.msg_keys,]
        ...
        if isinstance(self.di_warnlist, str): self.di_warnlist = [self.di_warnlist,]
    ...
    def format_UV_quest(self, uv_equation, device, reg, calibs, di_dt_def):
        ...
        def get_di_dict(dict_DI, dictAllcyphs):
            ...
            # есть ли ув_ди
            uv_fdi = dictAllcyphs[tmiparams['nameTMI']]['uv_fdi']
            return tmiparams, uv_fdi
        ...
        # парсинг значений
        def put_vals(ex, di_dict, uv_fdi, quest, di_dt_def):
            timequr = None
            type = None
            values = None
            range_type = ex[6]
            ...
            if args_prevcyph is not None:
                ...
                if args_prevcyph[4][1] < timequr[1]:
                    ...
                    pass
            quest[ex[3]] = ex[4] + [None, uv_fdi] + [timequr] + [None, None, None]
        ...
        for ex in reversed(patterns):
            ...
            # ex = calib_cust(ex)
            di_dict, uv_fdi = get_di_dict(dictDI, dictAllcyphs)
            grp_action, grp_value = frmt_eqrange(ex, calibs, grp_action, grp_value)
            ex_type_vals = put_vals(ex, di_dict, uv_fdi, quest, di_dt_def)
...
class QurDB():
    ...
    @classmethod
    def qur_tmi(cls, di_dict, ret_di, list_qur, clear_buf_uv):
        
        # потоки для послдовательных запросов к БД залочить зарос
        def _quest_db(key, key_args, lock, lockuv):
            def uv_di_send(*uvs): 
                # *kwargs
                if isinstance(uvs, int):
                    uvs = [uvs,]
                elif not(isinstance(uvs, (list, tuple))):
                    raise Exception('Ошибка в типе данных УВ_ДИ')
                for i in uvs:
                    SCPICMD(i)
                    bcolors.msg_def('Отпрвка УВ ДИ %X 3c' %i)
                    sleep(3)
                # **kwargs
                # if isinstance(kwargs['di'], int):
                #     kwargs['di'] = [kwargs['di'],]
                # elif not(isinstance(kwargs['di'], (list, tuple))):
                #     raise Exception('Ошибка в типе данных УВ_ДИ')
                # for i in kwargs['di']:
                #     SCPICMD(i)
                #     bcolors.msg_def('Отпрвка УВ ДИ %X' %i)
                #     sleep(3)
                
                # if isinstance(kwargs['ust'], int):
                #     kwargs['ust'] = [kwargs['ust'],]
                # elif not(isinstance(kwargs['ust'], (list, tuple))):
                #     raise Exception('Ошибка в типе данных УВ_ДИ')
                # for i in kwargs['ust']:
                #     SCPICMD(i)
                #     bcolors.msg_def('Отпрвка УВ УСТ %X' %i)
                #     sleep(3)
            ...
            while flag:
                ...
                # Выдача УВ
                if uvshecki is not None:
                    lockuv.acquire()
                    try:
                        uv_di_send(*uvshecki)
                        # eval('uv_di_send(%s)' %uvshecki)
                    except:
                        raise Exception('Ошибка выдачи УВ_ДИ \'%s\'' %key_di)
                    finally:
                        lockuv.release()
                # Запрос к БД
                try:
        ...
        # Один раз очистить буфер
        # Оттчиску буфера
        for i in di_dict.values():
            if i[3] is not None:
                SCPICMD(clear_buf_uv)
                print('УВ %X отчистка буфера 10с' %clear_buf_uv)
                sleep(10)
                break
        # Запуск потоков       
        for num, cyph in enumerate(list_qur):
            ...
            threads[num] = Thread(target=_quest_db, args=(cyph, cyph_args, lock, lockuv), daemon=True)
            threads[num].start()
        for th in threads:
            th.join()
...
class WrapEx():
    ...
    def ex_wait_v2(objUV, clear_buf_uv):
        ...
        # Опрос всех ДИ
        st = datetime.now()
        QurDB.qur_tmi(eq_range, ret_di, eq_lstord, clear_buf_uv)
