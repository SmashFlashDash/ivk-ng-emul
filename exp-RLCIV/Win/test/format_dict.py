...
di_fields = ('nameTMI', 'val_on', 'val_off', 'uv_fdi', 'n_bits', 'describe')
uv_fields = ('nameUV', 'uvhex', 't_TMI',  't_wait', 'msg_key', 'rev_devises',  'hex_data', 'rev_tmi', 'quest_uv')

# Главная функция переформатирвоания в слвоарь для json
def format_main(glb):
    # tags = ['_uv','_di','_msg','_di_keys', 'all_cyphs']  #тэги поиск словарей блоков
...
def format_dict(glb, tags, msg_fields, di_fields, uv_fields):
    ...
    for elem in s_uv:
        ...
        for key in copy:
            ...
            glb[elem][key][uv_fields[1]] = copy[key][1]     # nameUV
            # glb[elem][key][uv_fields[2]] = copy[key][2]     # typeUV
            if isinstance(copy[key][2], (int, float, None)):
                glb[elem][key][uv_fields[2]] = copy[key][3] # t_TMI  
            else:
    ...
    for elem in s_di:
        ...
        for key in copy:
            ...
            row = {
                ...
                di_fields[3]: copy[key][3],     # 'uv_fdi'
                di_fields[4]: copy[key][4],     # 'n_bits'
                di_fields[5]: copy[key][5]      # 'describe'
                }
            # Запись в словари
            # glb[elem + tags[3][3:]][name_di] = key
            glb[tags[-1]][name_di] = row
            glb[elem][key] = row
        # Сортировка
        # OrderedDict(sorted(glb[elem + '_keys'].items(), key=lambda t: t[1]))
        OrderedDict(sorted(glb[elem].items(), key=lambda t: t[0]))
...
def format_dictMain(glb, tags, title):    
    ...
    for key in copy:
        if isinstance(copy[key], dict):
            ...
            dictMain[key] = copy[key]
            print('Added raw row::: %s' %copy[key])
